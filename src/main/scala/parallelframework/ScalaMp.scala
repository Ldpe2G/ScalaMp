      ...
      case Some(num) => {
        ...
        operation match {
          case OpSignal(op) => {
            ...
            val result = temp_workers.map { case (id, worker) =>
              worker ? OperationTask(threadNum, op)
            }
        ...
        val result = schedule match {  
          case static(num) => {
            if (num <= 0) {
              ...
              val chushu = size / num
              val yushu = size % num
              val tmp = for (i <- 0 until chushu) yield r drop (i * num) take num
          ...
          case dynamic(num) => {
            ...
            val tmp = for (i <- 0 until chushu) yield r drop (i * internal) take internal
